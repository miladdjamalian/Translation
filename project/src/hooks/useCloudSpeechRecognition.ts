...
export const useCloudSpeechRecognition = (language: string) => {
  ...
  const processAudioChunk = useCallback(async (audioBlob: Blob) => {
    ...
    try {
      console.log('📤 Sending audio chunk to Google backend:', audioBlob.size, 'bytes, language:', getLanguageCode(language)); // Debug log
      ...
      console.log('📥 Google Speech API response status:', response.status, response.ok); // Debug log

      if (!response.ok) {
        const errorText = await response.text();
        console.error('❌ Google Speech API error response:', errorText); // Debug log
        throw new Error(`Google Speech API error: ${response.statusText} - ${errorText}`);
      }

      const result = await response.json();
      console.log('📋 Google Speech API result:', result); // Debug log
      
      if (result.transcript && result.transcript.trim()) {
        console.log('✅ Google Speech result:', result.transcript);
  ...
  const startListening = useCallback(async () => {
    try {
      console.log('🎤 Starting Google Cloud speech recognition for language:', getLanguageCode(language)); // Debug log
      ...
      console.log('✅ Got media stream for Google speech recognition with 48kHz sample rate:', stream); // Debug log
      ...
      console.log('🎵 Using MIME type for Google speech recognition:', mimeType); // Debug log
      ...
      mediaRecorder.onstart = () => {
        ...
        setInterimTranscript('');
        accumulatedTranscriptRef.current = '';
        console.log('🔴 Google Speech recognition started with 48kHz sample rate');
      };
      ...
      console.error('❌ Failed to start Google Cloud speech recognition:', error);
      throw error;
    }
  }, [language, processAudioChunk]);

  const stopListening = useCallback(() => {
    console.log('🛑 Stopping Google Cloud speech recognition'); // Debug log
